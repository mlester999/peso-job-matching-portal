...
const updateInfo = (applicationId) => {
    if (page.props.auth.user.employer) {
        router.get(`/employer/qualified/view/${applicationId}`);
    } else if (page.props.auth.user.admin) {
        router.get(`/admin/qualified/view/${applicationId}`);
    }
}
...
watch(
    ...
    debounce((value) => {
        ...
        if (page.props.auth.user.employer) {
            router.get(`/employer/qualified`, query, {
                preserveState: true,
                replace: true,
            });
        } else if (page.props.auth.user.admin) {
            router.get(`/admin/qualified`, query, {
                preserveState: true,
                replace: true,
            });
        }
...
<template>
    <AuthenticatedLayout title="Qualified">
        <template #header>
            <div class="px-4">
