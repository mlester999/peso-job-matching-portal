...
const updateInfo = (applicationId) => {
    if (page.props.auth.user.employer) {
        router.get(`/employer/for-deployment/view/${applicationId}`);
    } else if (page.props.auth.user.admin) {
        router.get(`/admin/for-deployment/view/${applicationId}`);
    }
}
...
watch(
    ...
    debounce((value) => {
        ...
        if (page.props.auth.user.employer) {
            router.get(`/employer/for-deployment`, query, {
                preserveState: true,
                replace: true,
            });
        } else if (page.props.auth.user.admin) {
            router.get(`/admin/for-deployment`, query, {
                preserveState: true,
                replace: true,
            });
        }
...
<template>
    <AuthenticatedLayout title="For Deployment">
        <template #header>
            <div class="px-4">
