...
const updateInfo = (applicationId) => {
    if (page.props.auth.user.employer) {
        router.get(`/employer/deployed/view/${applicationId}`);
    } else if (page.props.auth.user.admin) {
        router.get(`/admin/deployed/view/${applicationId}`);
    }
}
...
watch(
    ...
    debounce((value) => {
        ...
        if (page.props.auth.user.employer) {
            router.get(`/employer/deployed`, query, {
                preserveState: true,
                replace: true,
            });
        } else if (page.props.auth.user.admin) {
            router.get(`/admin/deployed`, query, {
                preserveState: true,
                replace: true,
            });
        }
...
<template>
    <AuthenticatedLayout title="Deployed">
        <template #header>
            <div class="px-4">
